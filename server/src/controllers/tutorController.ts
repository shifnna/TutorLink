  ...
  getPresignedUrl = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res
        ...
        .json({ success: false, message: "Failed to get presigned URL" });
    }
  };


  applyForTutor = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as any).user.id;
      const result = await this._tutorService.applyForTutor(userId, req.body);
      res
        .status(STATUS_CODES.CREATED)
        .json({ message: "Tutor profile created", result });
    } catch (error: any) {
